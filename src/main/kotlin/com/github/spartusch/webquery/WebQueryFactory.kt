...
object WebQueryFactory {
    ...
    private fun replacePlaceholders(uriWithPlaceholders: String,
                                    params: List<WebQueryParameter>) = params.fold(uriWithPlaceholders) {
        uri, param -> uri.replace("{${param.name}}", "[\"${param.defaultValue}\",\"${param.description}\"]")
    }

    @JvmStatic
    fun create(uriWithPlaceholders: String,
               vararg params: WebQueryParameter): WebQuery {
        return create(uriWithPlaceholders = uriWithPlaceholders, params = params.toList())
    }

    @JvmStatic
    fun create(uriWithPlaceholders: String,
               properties: Map<String, String> = mapOf("Formatting" to "None"),
               params: List<WebQueryParameter> = emptyList()): WebQuery {
